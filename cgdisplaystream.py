...
class CGDisplayStreamDelegate:
    ...
    def __init__(self, display, pixel_format, bounds, delay):
        ...
        if display not in displays:
            msg = f'Display with index {display} does not found, code: {self._kCGErrorIllegalArgument.__name__}'
            raise IndexError(msg)

        if pixel_format not in _pixelFormats:
            msg = f'Pixel format {pixel_format} is not supported, code: {self._kCGErrorIllegalArgument.__name__}'
    ...
    def run(self, filename):
        ...
        def handler(status: Quartz.CGDisplayStreamFrameStatus,
                    display_time, io_surface: IOSurfaceRef, updates: Quartz.CGDisplayStreamUpdateRef):
        ...
        if not extn:
            msg = f'No extension found for {filename}, code: {self._kCGErrorIllegalArgument.__name__}'
            raise ValueError(msg)
